...
/* 모든 요소를 캔버스(층) 안으로 제한 */
function clampToFloor(it){ const f=curFloor(); if(!f||f.w==null||it.x==null)return;
  if(it.w>f.w)it.w=f.w; if(it.h>f.h)it.h=f.h;
...
wrap.addEventListener('pointerdown',e=>{
  ...
  if(measMode){ if(e.button!==0)return; const hit=e.target.closest('[data-mi]');
    if(hit){ if(editMode){ const mi=STATE.items.find(x=>x.id===hit.dataset.mi); const r=wrap.getBoundingClientRect(); const cp={x:(e.clientX-r.left-view.tx)/view.s,y:(e.clientY-r.top-view.ty)/view.s};
...
wrap.addEventListener('pointermove',e=>{
  ...
  if(drag.mode==='pan'){ view.tx=drag.tx+(e.clientX-drag.sx); view.ty=drag.ty+(e.clientY-drag.sy); applyView(); }
    ...
    drag.ids.forEach(id=>{ const it=STATE.items.find(x=>x.id===id); it.x=snap(drag.orig[id].x+dx); it.y=snap(drag.orig[id].y+dy); clampToFloor(it);
...
document.addEventListener('keydown',e=>{
  ...
  if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key) && selected.size && !typing){ e.preventDefault();  // 방향키 미세조절
    ...
    selected.forEach(id=>{ const it=STATE.items.find(x=>x.id===id); if(it){ it.x+=dx; it.y+=dy; clampToFloor(it); } }); markDirty(); render(); paintSel(); return; }
...
function pasteClip(){ if(!clip.length)return; selected.clear();
  clip.forEach(c=>{ const it=JSON.parse(JSON.stringify(c)); it.id=uid(); it.floorId=curId(); it.x=snap(it.x+24); it.y=snap(it.y+24);
    if(it.type==='desk'){ const sc=(curFloor().mX)||0.0125, dn=it.dn||1, L=Math.round((it.mmU||1400)/1000*dn/sc), Sh=Math.round((it.mmD||700)/1000/sc); if((it.orient||'h')==='v'){it.w=Sh;it.h=L;}else{it.w=L;it.h=Sh;} }
    clampToFloor(it); STATE.items.push(it); selected.add(it.id); });
